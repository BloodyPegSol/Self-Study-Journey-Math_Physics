...
\begin{solution}
  Note that the set $\mathcal{V}\subseteq \mathbb{C}^{3}$ over $\mathbb{C}$ has the same linear operations with their required properties for a vector space. Hence, we must only check if $\mathcal{V}$ is closed under these linear operations and $0\in \mathcal{V}$ to conclude whether it is a vector space or not. \\
